<nav class="side-navbar">
      <div class="side-navbar-wrapper">
        <!-- Sidebar Header    -->
        <div class="sidenav-header d-flex align-items-center justify-content-center">
          <!-- User Info-->
          <div class="sidenav-header-inner text-center"><img src="{{ asset('img/avatar-7.jpg') }}" alt="person" class="img-fluid rounded-circle">
            <h2 class="h5">Rezeal Textile</h2><span>Tagline</span>
          </div>
          <!-- Small Brand information, appears on minimized sidebar-->
          <div class="sidenav-header-logo"><a href="index.html" class="brand-small text-center"> <strong>R</strong><strong class="text-primary">T</strong></a></div>
        </div>
        <!-- Sidebar Navigation Menus-->
        <div class="customer-menu">
           <!-- customer menu -->
          <h5 class="sidenav-heading">CUSTOMER</h5>
          <ul id="side-main-menu" class="side-menu list-unstyled">                  
            <li><a href="#"> <i class="fa fa-plus-square"></i> Add Order </a></li>
            <li><a href="#"> <i class="fa fa-list-ol"></i> Order List</a></li>
            <li><a href="#"> <i class="fa fa-file"></i> Invoice</a></li>
            <li><a href="#"> <i class="fa fa-file-text-o"></i>Receipt</a></li>
          </ul>
        </div>
        <div class="department-menu">
           <!-- department menu -->
          <h5 class="sidenav-heading">DEPARTMENT</h5>
          <ul id="side-main-menu" class="side-menu list-unstyled">     
            <li><a href="#"> <i class="fa fa-list-ol"></i> Order List</a></li>
            <li><a href="#"> <i class="fa fa-list"></i> Job List</a></li>
            <li><a href="#"> <i class="fa fa-tachometer"></i> Performance</a></li>
            <li><a href="#"> <i class="fa fa-file-text"></i>  Leave Application</a></li>                      
          </ul>
        </div>
        <div class="admin-menu">
          <h5 class="sidenav-heading">ADMIN/MANAGER</h5>
          <ul id="side-admin-menu" class="side-menu list-unstyled"> 
            <li><a href="#manageCustomerDropdown" aria-expanded="false" data-toggle="collapse"> <i class="fa fa-user"></i> Manage Customer </a>
              <ul id="manageCustomerDropdown" class="collapse list-unstyled ">
                <li><a href="#">New Application</a></li>
                <li><a href="#">Add Agent</a></li>
              </ul>
            </li>
            <li><a href="#manageStaffDropdown" aria-expanded="false" data-toggle="collapse"> <i class="fa fa-child"></i> Manage Staff </a>
              <ul id="manageStaffDropdown" class="collapse list-unstyled ">
                <li><a href="#">New Application</a></li>
                <li><a href="#">Add Staff</a></li>
                <li><a href="#">Leave</a></li>
                <li><a href="#">Leave List</a></li>
                <li><a href="#">Leave Setting</a></li>
                <li><a href="#">Performance</a></li>
              </ul>
            </li>
            <li><a href="#orderDropdown" aria-expanded="false" data-toggle="collapse"> <i class="fa fa-plus-square"></i> Order </a>
              <ul id="orderDropdown" class="collapse list-unstyled ">
                <li><a href="#">Order Setting</a></li>
                <li><a href="#">Order List</a></li>
              </ul>
            </li>
            <li> <a href="#"> <i class="icon-screen"> </i>Manage Stock </a></li>
            <li><a href="#invoiceDropdown" aria-expanded="false" data-toggle="collapse"> <i class="fa fa-file"></i> Invoice </a>
              <ul id="invoiceDropdown" class="collapse list-unstyled ">
                <li><a href="#">Pending</a></li>
              </ul>
            </li>
            <li><a href="#receiptDropdown" aria-expanded="false" data-toggle="collapse"> <i class="fa fa-file-text-o"></i> Receipt </a>
              <ul id="receiptDropdown" class="collapse list-unstyled ">
                <li><a href="#">Pending</a></li>
              </ul>
            </li>
            <li> <a href="#"> <i class="fa fa-money"> </i>Sale</a></li>
          </ul>
        </div>

      </div>
    </nav>