            ...
            <div class="card">
                ...
                <div class="card-body">
                                    ...
                                    @foreach($units->where('o_id',$spec->o_id)->where('s_id',$spec->s_id) as $unit)
                                           <tr>
                                              ...
                                              <td>
                                                  ...
                                                  <button 
                                                        class="btn btn-primary done" data-target="#Modal" data-uid="{{Auth::id()}}" data-process="deliver"
                                    ...
                                    @foreach($units->where('o_id',$spec->o_id)->where('s_id',$spec->s_id) as $unit)
                                           <tr>
                                              ...
                                              <td>
                                                  @if($units->where('o_id',$spec->o_id)->where('un_id',$unit->un_id)->where('un_status','3')->count()>0)
                                                  <button 
                                                        class="btn btn-primary done" data-target="#Modal" data-table="body" data-uid="{{Auth::id()}}" data-process="deliver"
                                                        data-oid="{{$unit->o_id}}" data-unid="{{$unit->un_id}}">Completed
                                                  </button>
                                                  @else
                                                  <input type="checkbox" name="jobdone" value="" checked="" disabled="">
                            ...
                            @endif
                             <br><br>
                            @if($no==$completed)
                            <form method="post" action="{{route('update_delivery')}}">@csrf
...
<script type="text/javascript">
    ...
    $(".done").click(function(e){
       ...
       // console.log(unid,oid,uid,process);
        ...
        $.ajax({
           type:'POST',
           url:"{{url('department/job_delivery')}}",
           data:formData,
